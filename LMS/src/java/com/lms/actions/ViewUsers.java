...
public class ViewUsers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<title>Users | Library Management System</title>");            
            out.println("</head>");
            ...
            out.println("<div id=\"head2\"><p>Users</p></div>");
            try {
                //connect to db
                ...
                if (rs.isBeforeFirst()) {
                    out.println("<table id=\"table\"><tr>"
                            + "<th class=\"bid\">User ID</th>"
                            + "<th>User Name</th>"
                            + "</tr>");
                    while (rs.next()) {
                    out.println("<tr><td class=\"bid\">" + rs.getInt(1) + "</td><td>" + rs.getString(2) + "</td></tr>");
                    }   
                    out.println("</table>");
                }
                con.close();
